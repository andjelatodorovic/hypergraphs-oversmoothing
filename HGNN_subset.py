...
if __name__ == "__main__":
    ...
    # Experiments with L' for various t values
    t_values = [0, 0.001, 1, 100]
    for t in t_values:
        print(f"Running experiment with L' (exponential normalized Laplacian, t={t}).")
        gnn_model_L_prime, accuracy_L_prime, dirichlet_energies_L_prime = hgnn_experiment(
            dataset_lcc, train_indices, test_indices, args, num_layers, t=t, use_exponential=True, early_stopping=False
        )
        dirichlet_energies_experiments[f"L_prime_t_{t}"] = dirichlet_energies_L_prime

    # Plot Dirichlet energy comparisons
    print("Plotting Dirichlet energy comparisons.")
    plot_dirichlet_energies_log_values(dirichlet_energies_experiments)
